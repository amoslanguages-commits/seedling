    ...
    if (file is File && file.path.endsWith('.dart')) {
      ...
      for (final match in matches) {
        ...
        if (block.contains('colors:') && !block.contains('stops:')) {
            final colorsStart = block.indexOf('colors:');
            // Rough check for > 2 commas after "colors:" inside the block
            final checkStr = block.substring(colorsStart);
            final commas = checkStr.split(',').length - 1;
            if (commas > 2) { 
